...
# Packages
pkgs <- c(
  "tidyverse", "tidymodels", "here", "Cairo", "colorspace",
...
gg_id_drink2 <- t_numerical %>%
  ...
  labs(
    title = "The ingredient-light drinks (grouped by drink id)", x = NULL, y =
      NULL
  )
...
data_tidy <- data %>%
  ...
  mutate(
    measure_vol_frc = str_extract(measure, "^([0-9]*(\\s?\\d*\\/\\d)?)"),
  ...
  mutate(
    measure_vol_frc = na_if(x = measure_vol_frc, y = ""),
...
gg_raw_measure <- (ggplot(data = data_tidy) +
  ...
  plot_annotation(
    subtitle = "Comparison of measure in raw and scaled units",
...
(gg_heat_drink_complexity <- only_gin_data %>%
  ...
  labs(
    title = "Complexity of gin drinks",
...
(gg_heat_gin <- no_gin_data %>%
  ...
  labs(
    title = "Common ingredients in gin drinks",
...
(gg_gin_importance_ml <- only_gin_data %>%
  ...
  labs(
    title = "Gin volume in the drinks",
    x = NULL, y = NULL, caption = "by @m_cadek; #TidyTuesday 2020"
  ) +
  coord_flip() +
  dark_theme(axis.text.x = element_text(
    colour = "white",
    angle = 90,
    hjust = 1,
    vjust = 0
...
set.seed(2020)
(gg_aggregated <- ggplot(aggregated_gin, aes(
  y = aprx_sum_volume_ml, x = aprx_avg_ingredient_volume_ml,
  ...
  labs(
    title = "Aggregated data about gin drinks",
    subtitle =
...
(gg_predicted <- ggplot(data_predicted, aes(
  y = aprx_sum_volume_ml, x = aprx_avg_ingredient_volume_ml,
  colour = .cluster
)) +
...
(gg_centres <- gg_predicted + geom_point(
  ...
  guides(
    colour = guide_legend("Clusters \n& centers"),
    fill = guide_legend("Clusters \n& centers")
  ))

# Visualise the predicted clusters with labels
row_names_labels <- c(
  "Gin Swizzle", "Ace", "Negroni", "White Lady", "Gin Fizz", "Casino", "Martini", "Casino Royale", "Pink Gin",
  "Abbey Cocktail", "Cherry Electric Lemonade", "Dragonfly", "Orange Oasis", 'French "75"'
)

set.seed(2020)
(gg_predicted_labels <- ggplot(data_predicted, aes(
  y = aprx_sum_volume_ml, x = aprx_avg_ingredient_volume_ml,
  colour = .cluster
)) +
  geom_jitter(size = 3, alpha = 0.5, width = 2, height = 2) +
...
(gg_elbow <- ggplot(clusters_statistics, aes(k, tot.withinss)) +
  ...
  dark_theme() +
  labs(
    subtitle = "Elbow method estimating the optimal number of clusters",
